...
public class TestStunEnemyAI : LivingEntity
{
    public enum State {Wander, Chase, Attack, ReturnToStart, Stunned};
    State currentState;
    ...
    {
        //AudioManagerScript.instance.PlaySound("Impact", transform.position);
        if(damage >= health && !isDead){
            if(OnDeathStatic != null){
                OnDeathStatic();
            }
            //AudioManagerScript.instance.PlaySound("Enemy Death", transform.position);
